...
class ArmoryExamplesBrowser {
	static main() {
		window.addEventListener("load",function(e) {
			...
			let build = { os : "Linux", commit : null, time : "2023-03-01"};
			if(build == null) {
			...
			let value2 = ArmoryExamplesBrowser.addProjectGroup("examples",["animation_actions","animation_blend","animation_bonechild","animation_instanced","animation_movebone","animation_timeline","animation_uv","call_hx","call_js","debug_draw","ease","file_read","file_storage","file_write","game_bowling","graphics_settings","input_mouselock","input_multitouch","input_sensor","instancing","light_area","light_ies","light_probes","light_probes_cubemap","light_probes_plane","light_volumetric","load_screen","lod","logic_Hello_World","logic_break","logic_callgroup","logic_camera_pan","logic_camera_zoom","logic_canvas","logic_event_fromhaxe","logic_event_global","logic_event_object","logic_gamepad","logic_gate","logic_get_contacts","logic_keyboard","logic_linked_variable","logic_loadurl","logic_mouselock","logic_object_rotate","logic_object_scale","logic_object_translate","logic_pause_trait","logic_pong","logic_scenes","logic_scenetree","logic_script","logic_set_property","logic_toy_car","logic_transform","macro_armpack","material_alpha","material_bake","material_batch","material_billboard","material_bump","material_decal","material_decal_colors","material_depth_texture","material_displace","material_glossy_metallic","material_normalmap","material_params","material_shadeless","material_shaders","material_sss","material_translucent","material_video","mesh_generate","mesh_import","mesh_terrain","navmesh","navmesh_follow","network_zui_chat","particle_bunny","particle_hair","particle_hair_animated","particle_info","particle_mesh","particle_smoke","physics_break","physics_collision_groups","physics_constraints","physics_drag","physics_pick","physics_ragdoll","physics_raycast","physics_softbody","render_bloom","render_colorgrading","render_splitscreen","render_to_texture","render_voxelao_teapots","scene_stream","screentex","script_camera_lerp","script_linkedgroup","script_properties","script_properties_global","script_rigidbody_trigger","script_spawnobject","script_transform","server_stream","sound","spawn_from_scene","text_area_string","tilesheet","tilesheet_2d","tilesheet_walkcycle","tween","ui_canvas","ui_events","ui_script2d","ui_script3d","wasm_call","wasm_trait_c","wasm_trait_rust","world_sun_direction"]);
			...
			window.onkeydown = function(e) {
				...
				case "f":
					...
					break;
				case "o":
					console.log("src/ArmoryExamplesBrowser.hx:148:","OPEN PROJECT");
					break;
				case "r":
